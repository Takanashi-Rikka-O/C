...
int time_to_exit=0;
pthread_mutex_t work_mutex;		//	 	mutually exclusive -- variable
...
//
	...
	res=pthread_mutex_init(&work_mutex,NULL);	//	 return 0 or error-code
	if (res != 0)
	{	
	...
	*/
	...
	pthread_mutex_lock(&work_mutex);	//	main-thread lock the work_mutex
	...
	{	
		...
		*/
		fgets(work_area,WORK_SIZE,stdin);
		pthread_mutex_unlock(&work_mutex);	//  main-thread unlock the work_mutex
		while (1)
		{
			pthread_mutex_lock(&work_mutex);	//	main-thread lock the work_mutex
			if (work_area[0] != '\0')
...
//
	...
	pthread_mutex_destroy(&work_mutex);	//	delete the mutually exclusive -- variable
	exit(EXIT_SUCCESS);
}
...
//	sem_wait(&bin_sem);	//	wait this bin_sem, if it is not 0, bin_sem-=1.. otherwise, wait this bin_sem is not 0
	...
	sleep(1);
	pthread_mutex_lock(&work_mutex);		//	thread locked the work_mutex
	...
	while (strncmp("end",work_area,3) != 0)
	{
		...
		work_area[0]='\0';
		pthread_mutex_unlock(&work_mutex);	//	thread unlocked the work_mutex
		sleep(1);
